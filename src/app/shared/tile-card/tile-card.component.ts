import { Component, Input, OnInit } from '@angular/core';
import { FACING } from 'src/app/core/constants';
import { SHAPES } from 'src/app/core/data';
import { Shape, Tile } from 'src/app/core/interfaces';

@Component({
  selector: 'app-tile-card',
  templateUrl: './tile-card.component.html',
  styleUrls: ['./tile-card.component.scss'],
})
export class TileCardComponent implements OnInit {
  @Input() tile: Tile;

  constructor() {}

  ngOnInit(): void {
    this.tile = this.reflectTheTile(this.tile);
    this.tile = this.rotateTheTile(this.tile);
  }

  // This should be moved to a service because I will need this for the grid later on
  reflectTheTile(tile: Tile): Tile {
    if (tile.reflection && tile.shape.reflective) {
      for (const row of tile.shape.layout.rows) {
        row.reverse();
      }
    }
    return tile;
  }

  rotateTheTile(tile: Tile): Tile {
    // up is the default, so it is not changed

    if (tile.facing === FACING.right) {
      this.rotateRight(tile);
    } else if (tile.facing === FACING.left) {
      this.rotateLeft(tile);
    } else if (tile.facing === FACING.down) {
      // 2 Right rotations make a downwards rotation
      this.rotateRight(tile);
      this.rotateRight(tile);
    }
    return tile;
  }

  /*
  Rotates with the following visual

    c1  c2 c3    Rotate Right    r3  r2 r1
  r1[ ][ ][ ]                  c1[ ][ ][ ]
  r2[ ][ ][ ]                  c2[ ][ ][ ]
  r3[ ][ ][ ]                  c3[ ][ ][ ]

  */
  rotateRight(tile: Tile): Tile {
    const rows = tile.shape.layout.rows;
    const newRows = [];

    let cellIndex = 0;
    let newRowIndex = 0;
    for (newRowIndex; newRowIndex < rows[0].length; newRowIndex++) {
      newRows.push([]);
      let rowIndex = rows.length - 1;
      for (rowIndex; rowIndex >= 0; rowIndex--) {
        newRows[newRowIndex].push(rows[rowIndex][cellIndex]);
      }
      cellIndex++;
    }

    tile.shape.layout.rows = newRows;
    // Probably need to remove the row1-3 properties from layout

    return tile;
  }

  rotateLeft(tile: Tile): Tile {
    const rows = tile.shape.layout.rows;
    const newRows = [];

    let cellIndex = rows[0].length - 1;
    let newRowIndex = 0;
    for (newRowIndex; newRowIndex < rows[0].length; newRowIndex++) {
      newRows.push([]);
      let rowIndex = 0;
      for (rowIndex; rowIndex < rows.length; rowIndex++) {
        newRows[newRowIndex].push(rows[rowIndex][cellIndex]);
      }

      cellIndex--;
    }

    tile.shape.layout.rows = newRows;
    return tile;
  }
}
